...
with tab1:
    provider = st.selectbox('Select model provider', ['AI21 Labs', 'Amazon', 'Anthropic', 'Cohere', 'Stability AI', 'Meta'])
    ...
    if provider == 'Meta':
        ...
        if pricing_option == 'On-Demand':
            ...
            input_price = 21.2
            output_price = 13.2
        input_tokens = st.number_input('Input tokens per request', min_value=0, max_value=1000000, value=1000)
        output_tokens = st.number_input('Output tokens per request', min_value=0, max_value=1000000, value=1000)
        ...
        if pricing_option == 'On-Demand':
            ...
            term = st.selectbox('Commitment term', ['None', '1 month', '6 months'])
        ...
        if pricing_option == 'On-Demand':
            ...
            input_tokens = st.number_input('Input tokens per request', min_value=0, max_value=1000000, value=11000)
            output_tokens = st.number_input('Output tokens per request', min_value=0, max_value=1000000, value=4000)
            customer_requests = st.number_input("Customer Requests per Day", min_value=0, max_value=1000000, value=500)
        ...
        model = st.selectbox('Select model', ['Command', 'Command-light'])

        input_tokens = st.number_input('Input tokens per request', min_value=0, max_value=1000000, value=6000)
        output_tokens = st.number_input('Output tokens per request', min_value=0, max_value=1000000, value=2000)
        customer_requests = st.number_input("Customer Requests per Day", min_value=0, max_value=1000000, value=500)
...
with tab2:
    ...
    if provider == 'Amazon OpenSearch Serverless Vector Store':
        ...
        indexing_OCU_pricing = (indexing_OCU * 720) * 0.24
        search_and_query_OCU_pricing = (search_and_query_OCU * 720) * 0.24
